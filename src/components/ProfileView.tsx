...
export const ProfileView = ({
  ...
  const { badges: allBadges } = useUserPerks();
  
  // Filter to only show equipped badges (or all if none equipped)
  ...
  const loadProfileData = async () => {
    ...
    try {
      ...
      const {
        data: profile
      } = await supabase.from('profiles').select('username, avatar_url, created_at, selected_flag, selected_continent, selected_clan, username_color, background_color, profile_border_style, equipped_badges').eq('user_id', user.id).single();
      if (profile) {
  ...
  return <>
      ...
      {showCustomization && (
        <ProfileCustomization
          ...
          currentEquippedBadges={equippedBadges}
